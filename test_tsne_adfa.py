...
def parse_args():
    ...
    parser.add_argument('--log_day_dir', type=str,
                       help='path to day pretrain model')
...
def test_simple(args):
    ...
    # LOADING DAY PRETRAINED MODEL:
    model_day_path = args.log_day_dir
    print(" Loading day pretrained encoder")
    day_encoder_path = os.path.join(model_day_path, "encoder.pth")
    
    print("   Loading pretrained encoder")
    day_encoder = networks.ResnetEncoder(18, False)
    day_loaded_dict_enc = torch.load(day_encoder_path, map_location=device)
    
    # extract the height and width of image that this model was trained with
    feed_height = day_loaded_dict_enc['height']
    feed_width = day_loaded_dict_enc['width']
    filtered_dict_enc = {k: v for k, v in day_loaded_dict_enc.items() if k in day_encoder.state_dict()}
    day_encoder.load_state_dict(filtered_dict_enc)
    day_encoder.to(device)
    day_encoder.eval()
    ...
    with torch.no_grad():
        ...
        for idx, line in enumerate(day_lines):
            ...
            image_path = "../Dataset/oxfordrobocar/" + folder_path + "/" + image_name + ".png"
            ...
            feature = day_encoder(input_image)[-1]
